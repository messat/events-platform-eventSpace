  ...
  if(isError){
    ...
    return (<Box role="main">
        ...
        <Grid2 container spacing={2} sx={{marginX: {xs: "2.5em"}, marginTop: "2.7em", marginBottom: "2.5em"}}>
            ...
            <Grid2 size={{ xs: 12, sm: 6, md: 4, lg: 3}} key={event._id} role="article">
                    
                        <Paper elevation={6} spacing={5} item="true" sx={{maxHeight: "700px", minHeight: "465px", mb: 2}}>

                        <img src={event.event_img_url} alt={`${event.title} event image`} className='eventImage' 
                        onError={(e) => {
                        ...
                        <div style={{display: "flex", flexDirection: "row", justifyContent: "flex-end"}}>
                            <Typography sx={{display: "inline-block", mr: 2, mt: -0.4, typography: { xs: 'h6', sm: 'body1', md: 'button'}}} variant='h6' tabIndex={0}>{event.spaces + " spaces"}</Typography>
                            <Groups2Icon  sx={{mr: 2, display: "inline-block", mt: {xs: 0.1, sm: -0.5}}} aria-label='Spaces Icon'></Groups2Icon> 
                        </div>
                        </Box>
                ...
                <Typography variant='button' sx={{align: "center", fontSize: "20px", color : 'red'}} role="alert">
